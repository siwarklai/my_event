...
router.post("/", async (req, res) => {
	try {
		...
		if (user.status === 'off') {
			return res.status(403).send({ message: "Le compte est verrouillé en raison de plusieurs tentatives de connexion infructueuses. Veuillez contacter l'assistance." });
		}
