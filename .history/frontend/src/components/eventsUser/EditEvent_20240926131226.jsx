  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8080/api/events/${id}`)
      ...
      .catch((error) => {
        ...
        window.alert(`Une erreur s'est produite lors de la récupération des détails de l'événement. Veuillez réessayer ultérieurement.`);
        console.log(error);
      });
  }, [id, enqueueSnackbar]);
    ...
    axios
      ...
      .then(() => {
        ...
        window.alert(`L'événement a été mis à jour avec succès`);
      ...
      .catch((error) => {
        ...
        window.alert('Une erreur s'est produite lors de la mise à jour de l'événement);
        console.log(error);
      });
  };
