  ...
  const handleSaveEvent = () => {
    ...
    axios
      .post('http://localhost:8080/api/events', data, {
        headers: {
          Authorization: `Bearer ${token}`, // Add Authorization token to the request
        },
      })
      .then(() => {
        setLoading(false);
        enqueueSnackbar('Event created successfully', { variant: 'success' });
        window.alert('Événement créé avec succès');
        navigate('/user/events');
      })
      .catch((error) => {
        setLoading(false);
        enqueueSnackbar('Error occurred during event creation', { variant: 'error' });
        window.alert('Une erreur est produite lors de la création de l`événement');
        console.log(error.response?.data?.message || error.message); // Log the error details
      });
  };
