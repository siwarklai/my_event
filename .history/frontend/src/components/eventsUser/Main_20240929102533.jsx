    ...
    return (
        <div className={styles.main_container}>
            ...
            <div className="card mb-3" style={{ maxWidth: '540px' }}>
                <img className="card-img-top" src="your-image-url.jpg" alt="Card image cap" />
                <div className="card-body">
                    <h5 className="card-title">Upcoming Event</h5>
                    <p className="card-text">
                        {upcomingEvent ? (
                            <>
                                <strong>{upcomingEvent.nom}</strong> - {upcomingEvent.type}
                                <br />
                                {upcomingEvent.lieu}, {upcomingEvent.codePostale}
                                <br />
                                Start Date: {new Date(upcomingEvent.dateDebut).toLocaleDateString()}
                                <br />
                                End Date: {new Date(upcomingEvent.dateFin).toLocaleDateString()}
                                <br />
                                Budget: {upcomingEvent.budget} TND
                            </>
                        ) : (
                            loading ? <Spinner /> : <p>{error || 'No upcoming events found.'}</p>
                        )}
                    </p>
